...
$sql = "SELECT * FROM wework.message WHERE sender_id='$userid'";
...
if($_SERVER["REQUEST_METHOD"] == "POST") {
      ...
      if($message==null){
        ...
        $sql="INSERT INTO `wework`.`message` (`sender_id`,`reciever_id`,`message`) VALUES( '$sender_id', '$reciever_id', '$message');";
        $result=mysqli_query($db,$sql);
      }
...
<html lang="en">
  ...
  <body>
    ...
    <div class="container"  style="margin-top: 50px;">
      <h3><a href="sent_messages.php" class="btn btn-common">View Sent Messages</a><a href="recieved_messages.php" style="margin-left:25px; "class="btn btn-common">View Recieved Messages</a></h3>
      <form role="form" class="login-form" action="" method="POST">
        <div class="input-group-addon search-category-container">
          <h3>Send Message to:</h3>
          <div class="form-group">
            <select name="reciever_id" class="form-control" style="margin-top: 20px;">
            <?php
            $userid=$_SESSION['user_id'];
              $sql="SELECT user_id FROM job LEFT JOIN job_applicant ON job.id=job_applicant.job_id WHERE job.poster_id='$userid'";
              $result=mysqli_query($db,$sql);
              $i=0;
              while($row=$result->fetch_assoc()){
                $a=$row['user_id'];
                $sql2="SELECT * FROM user LEFT JOIN user_detail ON user.user_detail_id=user_detail.id WHERE user.id='$a'";
                $fname=mysqli_query($db,$sql2)->fetch_object()->first_name;
                $lname=mysqli_query($db,$sql2)->fetch_object()->last_name;
                echo "<option value=\"$a\">".$fname." ".$lname."</option>";
                $i++;
              }
            ?>
          </select>
          <br>
          <textarea class="form-control" rows="5" name="message" style="border-color: #fec239;"></textarea>
         <button class="btn btn-common" style="margin-top: 25px;" name="send">Send</button>
        </div>
        </div>
      </form>
    </div>
